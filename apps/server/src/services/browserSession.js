...
export class BrowserSession {
  ...
  async executeCredentialActionPlan({ plan, usernameValue, passwordValue }) {
    ...
    return this.page
      .evaluate(({ actionPlan, userValue, passValue }) => {
        ...
        if (!submitTriggered && activeField && (activeField.form || activeField.closest("form"))) {
  ...
  async submitAuthCredentials(credentials = {}) {
    ...
    for (let attemptIndex = 0; attemptIndex < maxAttempts; attemptIndex += 1) {
      ...
      const noFillableFields = !plan.fillUsername && !plan.fillPassword;
    ...
    const latestSubmission = attempts[attempts.length - 1]?.submission ?? {
      ok: false,
