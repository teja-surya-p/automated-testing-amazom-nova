...
import { aggregateFunctionalRunnerResult } from "../src/types/functional/functionalRunner.js";
...
test("aggregateFunctionalRunnerResult returns stable pass summary and flow count", () => {
  ...
  assert.equal(result.summary, "Functional smoke-pack passed with no assertion failures.");
...
test("aggregateFunctionalRunnerResult sorts issues by severity then step", () => {
  ...
  assert.equal(result.summary, "Functional run completed with 3 failing assertions.");
...
test("aggregateFunctionalRunnerResult prioritizes blockers and soft-pass summary text", () => {
  ...
  assert.equal(result.summary, "Functional run ended early due to blocker LOGIN_REQUIRED.");
