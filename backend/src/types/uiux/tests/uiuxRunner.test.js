...
import { UiuxRunner } from "../uiuxRunner.js";
...
test("uiux runner returns normalized issue output with evidence refs and step", () => {
  ...
  assert.equal(issue.evidenceRefs[0].viewport.width, 1280);
  assert.equal(issue.evidenceRefs[0].viewport.height, 720);
  assert.equal(typeof issue.explanation?.whatHappened, "string");
