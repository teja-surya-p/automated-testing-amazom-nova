...
const AUTH_ENTRY_PATTERNS =
  /\bsign in\b|\blog in\b|\blogin\b|\bcontinue with account\b|\baccess account\b|\bmember login\b|\baccount login\b|\bportal login\b/i;
...
export function classifyUiuxElement(element) {
  ...
  if (AUTH_ENTRY_PATTERNS.test(haystack)) {
    return {
      category: "LOW_RISK",
      suggestedAction: {
        type: "click",
      ...
      flags: { isSearchField: false, isNavigation: true, isMenuExpand: false },
      reason: "Authentication entry control"
    };
  }
